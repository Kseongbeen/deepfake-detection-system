...
def mtcnn_fun(img, min_size, factor, thresholds):
    pb_path = './resource/mtcnn.pb'
    if not os.path.exists(pb_path):
        raise FileNotFoundError(f"MTCNNpb 파일이 존재하지 않습니다: {pb_path}")
...
# wrap graph function as a callable function if the model exists
if os.path.exists('./resource/mtcnn.pb'):
